public class Basket {

    private static int count = 0;
    private String items = "";
    private static int totalPrice = 0;
    private static int totalCount = 0;
    private int limit;
    private double totalWeight = 0;
    public static double averagePriseAll = 0;

    public Basket() {
        increaseCount(1);
        items = "Список товаров:";
        this.limit = 1000000;
    }

    public Basket(int limit) {
        this();
        this.limit = limit;
    }

    public Basket(String items, int totalPrice) {
        this();
        this.items = this.items + items;
        this.totalPrice = totalPrice;

    }
    private static void increaseCount(int count) {
        Basket.count = Basket.count + count;
    }
    public static int getCount() {
        return count;
    }
    public void add(String name, int price) {
        add(name, price, 1, 0);
    }

    public void add(String name, int price, int count, double weight) {
        add(name, price, count);
        items = items + " - " + weight * count + " g.";
        this.totalWeight = totalWeight + weight * count;
    }


    public void add(String name, int price, int count) {
        boolean error = contains(name);

        if (totalPrice + count * price >= limit) {
            error = true;
        }

        if (error) {
            System.out.println("Error occured :(");
            return;
        }

        items = items + "\n" + name + " - " +
            count + " шт. - " + price + " руб.";
        totalPrice = totalPrice + count * price;
        totalCount = totalCount + count;
        averagePriseAll = totalPrice / totalCount;
    }

    public void clear() {
        items = "";
        totalPrice = 0;
        totalWeight = 0;
    }

    public static double getAveragePriseAll() {
        return getAveragePriseAll();
    }
    public int getTotalPrice() {
        return totalPrice;
    }

    public double getTotalWeight() {
        return totalWeight;
    }

    public boolean contains(String name) {
        return items.contains(name);
    }


    public void print(String title) {
        System.out.println(title);
        if (items.isEmpty()) {
            System.out.println("Корзина пуста");
        } else {
            System.out.println(items);
            System.out.println("Общая стоимость: " + totalPrice + "руб." + "\n" +
                    "Общий вес: " + totalWeight + "g.");
            System.out.println("Общее количество товаров: " + totalCount);
            System.out.println("Средняя стоимость товаров: " + averagePriseAll);
        }
    }
}
